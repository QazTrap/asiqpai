...
app.post("/api/instrumental-orders", requireTelegramUser, async (req, res) => {
  try {
    ...
    const trackKey = instrumentalTitleKey(trackTitle);
    const configuredPrice = INSTRUMENTAL_ASIQ_PRICES.get(trackKey);
    const requestedPrice = Number(req.body?.priceAsiq);
    const priceAsiq = configuredPrice ?? requestedPrice;
    ...
    if (
      ...
      console.warn(
        ...
        JSON.stringify({ trackTitle, trackKey, requestedPrice })
      );
      return res.status(400).json({
