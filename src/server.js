...
const ARTIST_FILES_CONTACT = ARTIST_FILES_CONTACT_USERNAME
  ? `@${ARTIST_FILES_CONTACT_USERNAME}`
  : "администратору ASIQPAI";
const ARTIST_FILES_CONTACT_URL = ARTIST_FILES_CONTACT_USERNAME
  ? `https://t.me/${ARTIST_FILES_CONTACT_USERNAME}`
  : "";
...
const SUBMISSION_PRICES = Object.freeze({
  track: 5000,
  instrumental: 2500
});
const PRESALE_PRICE_STARS = 5000;
const PRESALE_PAYLOAD_PREFIX = "asiqpai-presale:";
const PRESALE_RESERVATION_MINUTES = 15;
const PRESALE_LOCK_ID = 731946215;
const PRESALE_NOTIFY_CHAT_ID =
  process.env.PRESALE_GROUP_CHAT_ID || ADMIN_CHAT_ID;
const TELEGRAM_WEBHOOK_PATH = "/telegram/webhook";

// ===== ASIQ POINTS =====
// Баланс хранится на сервере и привязан к Telegram user_id.
// Сервер сам определяет награду, чтобы клиент не мог подменить +1…+10.
const ASIQ_POINTS_MAX_TAPS_PER_SECOND = 12;
const ASIQ_POINTS_WINDOW_MS = 1000;
...
function instrumentalTitleKey(value) {
  return String(value || "")
    .normalize("NFKC")
    .trim()
    .replace(/\s+/g, " ")
...
// Цена инструменталов в ASIQ. Здесь можно назначать свою цену каждому биту.
// Сравнение названий устойчиво к регистру, лишним пробелам и Unicode-вариантам.
const INSTRUMENTAL_ASIQ_PRICES = new Map([
  [instrumentalTitleKey("Әңгіме"), 500]
]);
...
function getTelegramWebhookUrl() {
  ...
  return railwayDomain
    ? `https://${railwayDomain}${TELEGRAM_WEBHOOK_PATH}`
    : `https://asiqpai-production.up.railway.app${TELEGRAM_WEBHOOK_PATH}`;
}

function webhookSecretMatches(received) {
...
function presaleDisplayName(user) {
  return [user?.first_name, user?.last_name]
    .filter(Boolean)
    .join(" ")
    .trim() || user?.username || `Telegram ${user?.id}`;
}
...
function serializePresaleOrder(row) {
  return {
    orderId: row.order_id,
    status: row.status,
    active: row.status === "paid",
    passNumber: row.pass_number
      ? Number(row.pass_number)
      : null
  };
}

async function initPresalePassTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS presale_pass_orders (
      ...
      status TEXT NOT NULL DEFAULT 'pending'
        CHECK (status IN ('pending','approved','paid','expired','failed')),
      pass_number BIGINT UNIQUE,
      ...
      group_notification_error TEXT
    )
  `);

  await pool.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS presale_pass_one_paid_per_user
    ON presale_pass_orders (telegram_user_id)
    WHERE status = 'paid'
  `);

  await pool.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS presale_pass_one_reserved_or_paid_per_user
    ON presale_pass_orders (telegram_user_id)
    WHERE status IN ('approved','paid')
  `);

  await pool.query(`
    CREATE INDEX IF NOT EXISTS presale_pass_status_created_idx
    ON presale_pass_orders (status, created_at)
  `);
}

async function releaseExpiredPresaleReservations(client = pool) {
  await client.query(
    `UPDATE presale_pass_orders
     SET status = 'expired'
     WHERE status = 'approved'
       AND reserved_at < NOW() - ($1::text || ' minutes')::interval`,
    [String(PRESALE_RESERVATION_MINUTES)]
  );
}

async function getPresalePassState(telegramUserId) {
  await releaseExpiredPresaleReservations();

  const result = await pool.query(
    `SELECT order_id, status, pass_number
     FROM presale_pass_orders
     WHERE telegram_user_id = $1 AND status = 'paid'
     LIMIT 1`,
    [telegramUserId]
  );

  const activeOrder = result.rows[0] || null;

  return {
    active: Boolean(activeOrder),
    passNumber: activeOrder?.pass_number
      ? Number(activeOrder.pass_number)
      : null
  };
}

async function notifyPresaleGroup(order) {
  if (!PRESALE_NOTIFY_CHAT_ID) {
    console.warn(
      "PRESALE_GROUP_CHAT_ID and ADMIN_CHAT_ID are not configured"
    );
    return;
  }

  const username = order.username
    ? `@${escapeTelegramHtml(order.username)}`
    : "отсутствует";
  const orderShort = String(order.order_id)
    .split("-")[0]
    .toUpperCase();
  const message = [
    "💎 <b>ASIQPAI Presale — оплата получена</b>",
    "",
    `<b>Покупатель:</b> ${escapeTelegramHtml(order.display_name)}`,
    `<b>Username:</b> ${username}`,
    `<b>Telegram ID:</b> <code>${escapeTelegramHtml(order.telegram_user_id)}</code>`,
    `<b>Сумма:</b> 5 000 ⭐`,
    `<b>Presale Pass:</b> #${String(order.pass_number).padStart(6, "0")}`,
    `<b>Заказ:</b> <code>${escapeTelegramHtml(orderShort)}</code>`,
    "",
    "✅ Presale Pass активирован"
  ].join("\n");

  try {
    await telegramBotCall("sendMessage", {
      chat_id: PRESALE_NOTIFY_CHAT_ID,
      text: message,
      parse_mode: "HTML",
    ...
    await pool.query(
      `UPDATE presale_pass_orders
       SET group_notified_at = NOW(),
           group_notification_error = NULL
       WHERE order_id = $1`,
      [order.order_id]
    );
  } catch (error) {
    console.error("Presale group notification error:", error);

    await pool.query(
      `UPDATE presale_pass_orders
       SET group_notification_error = $2
       WHERE order_id = $1`,
      [
        order.order_id,
...
async function retryPresaleNotifications(limit = 25) {
  if (!PRESALE_NOTIFY_CHAT_ID) return;

  const result = await pool.query(
    `SELECT *
     FROM presale_pass_orders
     WHERE status = 'paid' AND group_notified_at IS NULL
  ...
  for (const order of result.rows) {
    await notifyPresaleGroup(order);
  }
}
...
function serializeArtist(row) {
  ...
  return {
    ...
    status: row.status,
    onboardingStatus: row.onboarding_status || "waiting_files",
...
async function sendArtistNotification({ telegramUser, artist }) {
  ...
  const text = [
    "👑 Новая оплаченная заявка артиста", "",
    `🎤 Имя артиста: ${artist.artistName}`,
    `🔢 Artist #${String(artist.artistNumber).padStart(4, "0")}`,
    `👤 Telegram: ${username}`,
    `🆔 Telegram ID: ${telegramUser.id}`,
    `👛 Кошелёк: ${artist.walletAddress}`,
    `💰 Оплачено: ${ARTIST_MEMBERSHIP_ASIQ.toLocaleString("en-US")} ASIQ`,
    "📂 Статус: ждём MP3 и обложку", "",
    username === "нет username"
      ? "Свяжитесь с артистом по Telegram ID."
      : `Напишите артисту ${username} и попросите отправить MP3 и обложку.`,
...
async function findInstrumentalAsiqPayment({ sourceWallet, comment, amountRaw, createdAt }) {
  const merchant = normalizeAddress(MERCHANT_WALLET);
  const source = normalizeAddress(sourceWallet);
  const createdUnix = Math.floor(new Date(createdAt).getTime() / 1000) - 60;
  const url = new URL(`https://tonapi.io/v2/accounts/${encodeURIComponent(merchant)}/events`);
  ...
  for (const event of Array.isArray(data?.events) ? data.events : []) {
    ...
    for (const action of Array.isArray(event?.actions) ? event.actions : []) {
      ...
      const sender = normalizeAddress(transfer.sender?.address || transfer.sender || transfer.source?.address || transfer.source);
      const recipient = normalizeAddress(transfer.recipient?.address || transfer.recipient || transfer.destination?.address || transfer.destination);
      const jettonMaster = normalizeAddress(transfer.jetton?.address || transfer.jetton || transfer.jetton_master?.address || transfer.jetton_master);
      const amount = BigInt(String(transfer.amount || "0"));
      const actionComment = String(transfer.comment || transfer.forward_payload?.text || "").trim();
      const successful = action?.status !== "failed" && event?.is_scam !== true;

      if (successful && sender === source && recipient === merchant &&
          jettonMaster === normalizeAddress(ASIQ_MASTER) &&
          amount >= BigInt(amountRaw) && actionComment === comment) {
        return { hash: event.event_id || event.trace_id || "", amount: amount.toString(), comment: actionComment };
      }
    }
  }
  return null;
...
function normalizeSubmissionType(value) {
  ...
  return Object.prototype.hasOwnProperty.call(SUBMISSION_PRICES, type) ? type : "";
}

function serializeSubmissionOrder(row) {
  return {
    orderId: row.id,
    status: row.status,
    artistName: row.artist_name,
    ...
    priceAsiq: Number(row.price_asiq),
    txHash: row.tx_hash || null,
    filesContact: ARTIST_FILES_CONTACT,
    filesContactUrl: ARTIST_FILES_CONTACT_URL
  };
}

async function sendSubmissionNotification({ telegramUser, order, payment }) {
  if (!ADMIN_CHAT_ID) return;

  const username = telegramUser.username
    ? `@${telegramUser.username}`
    : "username отсутствует";
  const fullName = `${telegramUser.first_name || ""} ${telegramUser.last_name || ""}`.trim() || "Не указано";
  const typeLabel = order.submission_type === "instrumental" ? "Instrumental" : "Track";

  const text = [
    "🎵 Новая заявка на публикацию ASIQPAI",
    "",
    `👤 Пользователь: ${fullName}`,
    `🔗 Username: ${username}`,
    `🆔 Telegram ID: ${telegramUser.id}`,
    "",
    `🎤 Артист: ${order.artist_name}`,
    `🎼 Название: ${order.work_title}`,
    `📂 Тип: ${typeLabel}`,
    `💰 Оплачено: ${order.price_asiq} ASIQ`,
    `📦 Заявка: ${order.id}`,
    "✅ Статус: оплачено, ожидаются файлы",
    "",
    username === "username отсутствует"
      ? `Свяжитесь по Telegram ID и запросите аудиофайл и обложку.`
      : `Запросите у ${username} аудиофайл и квадратную обложку.`,
    `🔐 Транзакция: ${payment.hash || "найдена в блокчейне"}`
  ].join("\n");
...
async function sendInstrumentalAsiqNotification({ telegramUser, order, payment }) {
  if (!ADMIN_CHAT_ID) return;
  const username = telegramUser.username ? `@${telegramUser.username}` : "username отсутствует";
  const fullName = `${telegramUser.first_name || ""} ${telegramUser.last_name || ""}`.trim() || "Не указано";
  const text = [
    "🛒 Новая покупка инструментала за ASIQ", "",
    `👤 Покупатель: ${fullName}`,
    `🔗 Username: ${username}`,
    `🆔 Telegram ID: ${telegramUser.id}`, "",
    `🎵 Инструментал: ${order.track_title}`,
  ...
  } catch (error) { console.error("Instrumental notification error:", error); }
}

async function initInstrumentalOrdersTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS instrumental_asiq_orders (
      id UUID PRIMARY KEY,
      telegram_id BIGINT NOT NULL,
      wallet_address TEXT NOT NULL,
      track_title TEXT NOT NULL,
      price_asiq NUMERIC(30, 9) NOT NULL,
...
async function initSubmissionOrdersTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS track_submission_orders (
      id UUID PRIMARY KEY,
      telegram_id BIGINT NOT NULL,
      wallet_address TEXT NOT NULL,
      artist_name TEXT NOT NULL,
      work_title TEXT NOT NULL,
      submission_type TEXT NOT NULL CHECK (submission_type IN ('track','instrumental')),
      price_asiq NUMERIC(30, 9) NOT NULL,
      amount_raw NUMERIC(40, 0) NOT NULL,
      comment TEXT NOT NULL UNIQUE,
      status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','paid','expired')),
  ...
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS track_submission_orders_user_idx
    ON track_submission_orders (telegram_id, created_at DESC)
  `);
}
...
// ASIQ POINTS
// =======================
async function initAsiqPointsTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS asiq_points_accounts (
      telegram_id BIGINT PRIMARY KEY REFERENCES users(telegram_id) ON DELETE CASCADE,
      balance BIGINT NOT NULL DEFAULT 0 CHECK (balance >= 0),
      total_taps BIGINT NOT NULL DEFAULT 0 CHECK (total_taps >= 0),
      tap_window_started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      tap_window_count INTEGER NOT NULL DEFAULT 0 CHECK (tap_window_count >= 0),
...
function rollAsiqPointsReward() {
  // 0..999. Маленькие награды выпадают чаще, +8…+10 — редко.
  const roll = crypto.randomInt(0, 1000);
  if (roll < 420) return 1;
  if (roll < 640) return 2;
  if (roll < 770) return 3;
  if (roll < 850) return 4;
  if (roll < 900) return 5;
  if (roll < 940) return 6;
  if (roll < 965) return 7;
  if (roll < 980) return 8;
  if (roll < 992) return 9;
  return 10;
}

function serializeAsiqPoints(row) {
  return {
    ok: true,
    balance: String(row?.balance ?? 0),
    totalTaps: String(row?.total_taps ?? 0),
    lastTapAt: row?.last_tap_at || null
  };
}

function publicAsiqPointsName(row) {
  ...
  const anonymousCode = crypto
    .createHash("sha256")
    .update(`asiqpai-points:${String(row?.telegram_id || "0")}`)
    .digest("hex")
    .slice(0, 6)
    .toUpperCase();

  return `Player ${anonymousCode}`;
}

async function ensureAsiqPointsAccount(client, telegramUserId) {
  await client.query(
    `INSERT INTO asiq_points_accounts (telegram_id)
     VALUES ($1)
...
app.get(
  ...
  async (req, res) => {
    try {
      return res.json(
        await getPresalePassState(req.telegramUser.id)
      );
    } catch (error) {
      console.error("Presale Pass status error:", error);
      return res.status(500).json({
        error: "Не удалось проверить Presale Pass"
      });
    }
  }
);
  ...
  async (req, res) => {
    try {
      const result = await pool.query(
        `SELECT order_id, status, pass_number
         FROM presale_pass_orders
         WHERE order_id = $1 AND telegram_user_id = $2
         LIMIT 1`,
        [req.params.orderId, req.telegramUser.id]
      );

      if (!result.rows[0]) {
        return res.status(404).json({
          error: "Счёт Presale Pass не найден"
        });
      }

      const passState = await getPresalePassState(
        req.telegramUser.id
      );

      return res.json({
        ...serializePresaleOrder(result.rows[0]),
        ...passState
      });
    } catch (error) {
      console.error("Presale order status error:", error);
      return res.status(500).json({
        error: "Не удалось проверить оплату Presale Pass"
      });
    }
  }
);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      const passState = await getPresalePassState(user.id);

      if (passState.active) {
        return res.json(passState);
      }
      ...
      if (existing.rows[0]) {
        return res.json({
          ...
          invoiceUrl: existing.rows[0].invoice_url,
          priceStars: PRESALE_PRICE_STARS,
          ...passState
        });
      }
      ...
      const invoicePayload =
        `${PRESALE_PAYLOAD_PREFIX}${orderId}`;

      await pool.query(
        `INSERT INTO presale_pass_orders (
           order_id,
           telegram_user_id,
           username,
           display_name,
      ...
      try {
        invoiceUrl = await telegramBotCall(
          "createInvoiceLink",
          {
            title: "ASIQPAI Presale Pass",
            description:
              "Digital Presale Pass for ASIQPAI.",
            payload: invoicePayload,
            currency: "XTR",
            prices: [
              {
                label: "ASIQPAI Presale Pass",
                amount: PRESALE_PRICE_STARS
              }
            ]
          }
        );
      ...
      console.error("Presale invoice error:", error);
      return res.status(500).json({
        error: "Не удалось создать счёт Telegram Stars"
      });
    }
  }
);

async function approvePresalePreCheckout(query) {
  const payload = String(query?.invoice_payload || "");

  if (!payload.startsWith(PRESALE_PAYLOAD_PREFIX)) {
    return false;
  }
  ...
  let approved = false;
  let errorMessage =
    "Этот Presale Pass сейчас недоступен";

  try {
    if (
      query.currency !== "XTR" ||
      Number(query.total_amount) !== PRESALE_PRICE_STARS
    ) {
      errorMessage =
        "Цена изменилась. Откройте новый счёт";
      throw new Error("Invalid Presale Stars amount");
    }
    ...
    await releaseExpiredPresaleReservations(client);

    const orderResult = await client.query(
      `SELECT order_id, telegram_user_id, status
       FROM presale_pass_orders
    ...
    if (
      ...
      errorMessage =
        "Счёт не принадлежит этому Telegram-профилю";
      throw new Error("Presale invoice owner mismatch");
    }

    if (order.status === "paid") {
      errorMessage = "Presale Pass уже оплачен";
      throw new Error("Presale Pass is already paid");
    }
    ...
    if (order.status !== "approved") {
      const existingPass = await client.query(
        `SELECT 1
         ...
         LIMIT 1`,
        [order.telegram_user_id, order.order_id]
      );

      if (existingPass.rows[0]) {
        errorMessage =
          "Для этого профиля Pass уже активирован";
        throw new Error(
          "Telegram user already has a Presale Pass"
        );
      }

      await client.query(
        `UPDATE presale_pass_orders
         SET status = 'approved', reserved_at = NOW()
         WHERE order_id = $1`,
        [order.order_id]
      );
    }
    ...
    console.warn("Presale pre-checkout rejected:", error.message);
  } finally {
    client?.release();
  }

  try {
    await telegramBotCall("answerPreCheckoutQuery", {
...
async function activatePaidPresalePass(message) {
  ...
  try {
    if (
      ...
      Number(payment.total_amount) !== PRESALE_PRICE_STARS ||
      !Number.isSafeInteger(telegramUserId) ||
      !chargeId
    ) {
      shouldRefund = Boolean(
        Number.isSafeInteger(telegramUserId) && chargeId
      );
      throw new Error("Invalid successful_payment data");
    }
    ...
    if (
      ...
      shouldRefund = true;
      throw new Error("Paid Presale order owner mismatch");
    }
      ...
      const existingPaid = await client.query(
        `SELECT order_id
         ...
         WHERE telegram_user_id = $1
           ...
           AND order_id <> $2
         LIMIT 1`,
        [telegramUserId, order.order_id]
      );

      if (existingPaid.rows[0]) {
        shouldRefund = true;
        throw new Error(
          "Telegram user already owns another Presale Pass"
        );
      }

      const passResult = await client.query(
        `SELECT COALESCE(MAX(pass_number), 0) + 1
           AS next_pass
         FROM presale_pass_orders
         WHERE status = 'paid'`
      );
      const nextPass = Number(
        passResult.rows[0]?.next_pass || 1
      );
    ...
    if (shouldRefund) {
      await telegramBotCall("refundStarPayment", {
        user_id: telegramUserId,
        telegram_payment_charge_id: chargeId
      }).catch(refundError => {
        console.error(
          "Presale automatic refund error:",
          refundError
        );
      });
      return true;
  ...
  if (paidOrder && !paidOrder.group_notified_at) {
    await notifyPresaleGroup(paidOrder);
  }

  return true;
}

async function handlePresaleTelegramUpdate(update) {
  if (update?.pre_checkout_query) {
    return approvePresalePreCheckout(
  ...
  if (message?.successful_payment) {
    return activatePaidPresalePass(message);
  }
...
app.post(TELEGRAM_WEBHOOK_PATH, async (req, res) => {
  ...
  if (!webhookSecretMatches(receivedSecret)) {
...
async function registerPresaleWebhook() {
  const webhookUrl = getTelegramWebhookUrl();

  if (!webhookUrl) {
    console.warn(
      "TELEGRAM_WEBHOOK_URL and RAILWAY_PUBLIC_DOMAIN are not configured; Presale Stars webhook was not registered"
    );
    return false;
  }
  ...
  try {
    ...
    if (currentArtist.rowCount) {
      ...
      return res.json({
        ...
        filesContact: ARTIST_FILES_CONTACT,
        filesContactUrl: ARTIST_FILES_CONTACT_URL,
        artist: serializeArtist(currentArtist.rows[0])
      });
    }
...
// Текущий серверный баланс ASIQ Points.
app.get(
  "/api/points",
  requireTelegramUser,
  async (req, res) => {
    try {
      await ensureAsiqPointsAccount(pool, req.telegramUser.id);

      const result = await pool.query(
        `SELECT balance, total_taps, last_tap_at
         FROM asiq_points_accounts
         WHERE telegram_id = $1`,
        [req.telegramUser.id]
      );

      return res.json(serializeAsiqPoints(result.rows[0]));
    } catch (error) {
      console.error("ASIQ Points status error:", error);
      return res.status(500).json({
        error: "Не удалось загрузить ASIQ Points"
      });
    }
  }
);

// TOP 10 ASIQ Points + место текущего пользователя.
app.get(
    ...
    try {
      ...
      const result = await pool.query(
        `WITH ranked AS (
           ...
           FROM asiq_points_accounts a
           LEFT JOIN users u
             ON u.telegram_id = a.telegram_id
         )
         SELECT *
         FROM ranked
         WHERE rank <= 10 OR telegram_id = $1
         ORDER BY rank ASC`,
        [req.telegramUser.id]
      );
      ...
      const top = rows
        ...
        .map(row => ({
          ...
          name: publicAsiqPointsName(row),
          balance: String(row.balance ?? 0),
          isMe: Number(row.telegram_id) === Number(req.telegramUser.id)
        }));

      const meRow = rows.find(
        row => Number(row.telegram_id) === Number(req.telegramUser.id)
      );
      ...
      return res.json({
        ...
        me: meRow
          ? {
              ...
              name: publicAsiqPointsName(meRow),
              balance: String(meRow.balance ?? 0)
            }
          : null,
        totalPlayers: Number(meRow?.total_players || rows[0]?.total_players || 0)
      });
    } catch (error) {
      console.error("ASIQ Points leaderboard error:", error);
      return res.status(500).json({
        error: "Не удалось загрузить рейтинг ASIQ Points"
      });
    }
  }
);

// Один подтверждённый тап. Награду выбирает только сервер.
app.post(
  "/api/points/tap",
  requireTelegramUser,
  async (req, res) => {
    ...
    try {
      await client.query("BEGIN");
      await ensureAsiqPointsAccount(client, req.telegramUser.id);
      ...
      const now = Date.now();
      const windowStarted = new Date(account.tap_window_started_at).getTime();
      const windowExpired =
        !Number.isFinite(windowStarted) ||
        now - windowStarted >= ASIQ_POINTS_WINDOW_MS;

      let windowCount = windowExpired
        ? 0
        : Number(account.tap_window_count || 0);

      if (windowCount >= ASIQ_POINTS_MAX_TAPS_PER_SECOND) {
        const retryAfterMs = Math.max(
          50,
          ASIQ_POINTS_WINDOW_MS - Math.max(0, now - windowStarted)
        );

        await client.query("ROLLBACK");
        res.set("Retry-After", String(Math.ceil(retryAfterMs / 1000)));
        return res.status(429).json({
          error: "Слишком много тапов. Попробуйте чуть медленнее.",
          retryAfterMs
        });
      }

      const reward = rollAsiqPointsReward();
      windowCount += 1;

      const updated = await client.query(
        `UPDATE asiq_points_accounts
         SET balance = balance + $2,
             total_taps = total_taps + 1,
             tap_window_started_at = CASE
               WHEN $3::boolean THEN NOW()
               ELSE tap_window_started_at
             END,
             tap_window_count = $4,
             last_tap_at = NOW(),
             updated_at = NOW()
         WHERE telegram_id = $1
         RETURNING balance, total_taps, last_tap_at`,
        [
          req.telegramUser.id,
          reward,
          windowExpired,
      ...
      return res.json({
        ...serializeAsiqPoints(updated.rows[0]),
        reward
      });
    } catch (error) {
      try {
      ...
      console.error("ASIQ Points tap error:", error);
      return res.status(500).json({
        error: "Не удалось начислить ASIQ Points"
      });
    } finally {
      client.release();
    }
...
app.post("/api/submissions", requireTelegramUser, async (req, res) => {
  try {
    const walletAddress = normalizeAddress(req.body?.walletAddress);
    const artistName = cleanArtistName(req.body?.artistName);
    const workTitle = cleanTrackTitle(req.body?.workTitle);
    const submissionType = normalizeSubmissionType(req.body?.submissionType);

    if (!walletAddress) {
      return res.status(400).json({ error: "Подключите TON-кошелёк" });
    }
    if (!artistName || artistName.length < 2) {
      return res.status(400).json({ error: "Введите имя артиста" });
    }
    if (!workTitle) {
      return res.status(400).json({ error: "Введите название работы" });
    }
    if (!submissionType) {
      return res.status(400).json({ error: "Выберите Track или Instrumental" });
    }

    const priceAsiq = SUBMISSION_PRICES[submissionType];
    const amountRaw = (
      BigInt(priceAsiq) * 10n ** BigInt(ASIQ_DECIMALS)
    ).toString();
    const id = crypto.randomUUID();
    const typeCode = submissionType === "instrumental" ? "INSTR" : "TRACK";
    const comment = `ASIQPAI-SUBMIT-${typeCode}-${id.slice(0, 10).toUpperCase()}`;
    const jettonWalletAddress = await getJettonWalletAddress(walletAddress);
    const payload = createJettonTransferPayload({
      amountRaw,
      recipient: MERCHANT_WALLET,
    ...
    const inserted = await pool.query(`
      INSERT INTO track_submission_orders
      (id, telegram_id, wallet_address, artist_name, work_title, submission_type, price_asiq, amount_raw, comment)
    ...
    return res.status(201).json({
      ...serializeSubmissionOrder(inserted.rows[0]),
      transaction: {
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{
          address: jettonWalletAddress,
    ...
    console.error("Submission order error:", error);
    return res.status(500).json({ error: "Не удалось создать заявку" });
  }
});

app.get("/api/submissions/:id", requireTelegramUser, async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const result = await client.query(`
      SELECT * FROM track_submission_orders
    ...
    if (order.status === "paid" || order.status === "expired") {
      await client.query("COMMIT");
      return res.json(serializeSubmissionOrder(order));
    }

    if (Date.now() - new Date(order.created_at).getTime() > ORDER_TTL_MS) {
      const expired = await client.query(`
        UPDATE track_submission_orders
        SET status = 'expired'
        WHERE id = $1
        RETURNING *
      `, [order.id]);
      await client.query("COMMIT");
      return res.json(serializeSubmissionOrder(expired.rows[0]));
    }

    const payment = await findInstrumentalAsiqPayment({
      sourceWallet: order.wallet_address,
      comment: order.comment,
    ...
    return res.json(serializeSubmissionOrder(paid.rows[0]));
  } catch (error) {
    try { await client.query("ROLLBACK"); } catch {}
    console.error("Submission verification error:", error);
    return res.status(500).json({ error: "Не удалось проверить оплату заявки" });
  } finally {
    client.release();
  }
});

app.post("/api/instrumental-orders", requireTelegramUser, async (req, res) => {
  try {
    const walletAddress = normalizeAddress(req.body?.walletAddress);
    const trackTitle = cleanTrackTitle(req.body?.trackTitle);
    if (!walletAddress) return res.status(400).json({ error: "Подключите TON-кошелёк" });
    const requestedPrice = Number(req.body?.priceAsiq);
    const priceAsiq = requestedPrice;

    if (
      !trackTitle ||
      !Number.isSafeInteger(priceAsiq) ||
      priceAsiq < 1 ||
      priceAsiq > 1_000_000_000
    ) {
      console.warn(
        "Invalid instrumental purchase:",
        JSON.stringify({ trackTitle, requestedPrice })
      );
      return res.status(400).json({
        error: "Для инструментала не указана корректная цена ASIQ"
      });
    }

    const id = crypto.randomUUID();
    const comment = `ASIQPAI-BEAT-${id.slice(0, 12).toUpperCase()}`;
    const amountRaw = (BigInt(priceAsiq) * 10n ** BigInt(ASIQ_DECIMALS)).toString();
    const jettonWalletAddress = await getJettonWalletAddress(walletAddress);
    const payload = createJettonTransferPayload({
      amountRaw,
      recipient: MERCHANT_WALLET,
    ...
    await pool.query(`
      INSERT INTO instrumental_asiq_orders
      (id, telegram_id, wallet_address, track_title, price_asiq, amount_raw, comment)
      VALUES ($1,$2,$3,$4,$5,$6,$7)
    `, [id, req.telegramUser.id, walletAddress, trackTitle, priceAsiq, amountRaw, comment]);
    ...
    return res.status(201).json({
      ...
      transaction: {
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{ address: jettonWalletAddress, amount: "80000000", payload }]
      }
    });
  } catch (error) {
    console.error("Instrumental ASIQ order error:", error);
    return res.status(500).json({ error: "Не удалось создать покупку за ASIQ" });
  }
});

app.get("/api/instrumental-orders/:id", requireTelegramUser, async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const result = await client.query(`
      SELECT * FROM instrumental_asiq_orders
      WHERE id=$1 AND telegram_id=$2 FOR UPDATE
    `, [req.params.id, req.telegramUser.id]);
    if (!result.rowCount) {
      await client.query("ROLLBACK");
    ...
    if (order.status === "paid" || order.status === "expired") {
      await client.query("COMMIT");
      return res.json({ orderId: order.id, status: order.status, trackTitle: order.track_title, priceAsiq: String(order.price_asiq), txHash: order.tx_hash || null });
    }
    if (Date.now() - new Date(order.created_at).getTime() > ORDER_TTL_MS) {
      await client.query(`UPDATE instrumental_asiq_orders SET status='expired' WHERE id=$1`, [order.id]);
      await client.query("COMMIT");
    ...
    await client.query(`UPDATE instrumental_asiq_orders SET status='paid', tx_hash=$2, paid_at=NOW() WHERE id=$1`, [order.id, payment.hash]);
    await client.query("COMMIT");
    await sendInstrumentalAsiqNotification({ telegramUser: req.telegramUser, order, payment });
    return res.json({ orderId: order.id, status: "paid", trackTitle: order.track_title, priceAsiq: String(order.price_asiq), txHash: payment.hash });
  } catch (error) {
    try { await client.query("ROLLBACK"); } catch {}
    console.error("Instrumental ASIQ verification error:", error);
    return res.status(500).json({ error: "Не удалось проверить оплату ASIQ" });
  } finally { client.release(); }
});
...
await initDatabase();
await initAsiqPointsTable();
await initDailyRewardTable();
await initWithdrawalsTable();
await initInstrumentalOrdersTable();
await initSubmissionOrdersTable();
await initPresalePassTable();
await retryPresaleNotifications();
...
verifyPayoutWallet()
  .then((info) => {
    ...
    app.listen(PORT, async () => {
      ...
      try {
        await registerPresaleWebhook();
      } catch (error) {
        console.error(
          "Telegram Presale webhook setup error:",
          error
        );
      }
    });
