...
app.post("/api/instrumental-orders", requireTelegramUser, async (req, res) => {
  try {
    ...
    const priceAsiq = INSTRUMENTAL_ASIQ_PRICES.get(trackKey);
    if (!priceAsiq) {
      console.warn("Unknown instrumental title:", JSON.stringify(trackTitle), "key:", trackKey);
      return res.status(400).json({ error: "Этот инструментал не продаётся" });
    }
