...
const SUBMISSION_FILES_CONTACT_USERNAME = String(
  process.env.SUBMISSION_FILES_CONTACT_USERNAME || "asiqpai"
).replace(/^@+/, "").trim();
const SUBMISSION_FILES_CONTACT = SUBMISSION_FILES_CONTACT_USERNAME
  ? `@${SUBMISSION_FILES_CONTACT_USERNAME}`
  : "администратору ASIQPAI";
const SUBMISSION_FILES_CONTACT_URL = SUBMISSION_FILES_CONTACT_USERNAME
  ? `https://t.me/${SUBMISSION_FILES_CONTACT_USERNAME}`
  : "";
...
const SUBMISSION_PRICES = Object.freeze({ track: 5000, instrumental: 2500 });
const JETTON_TRANSFER_OP = 0x0f8a7ea5;
const SUBMISSION_FORWARD_TON = 1n;
...
function submissionPrice(type) {
  return SUBMISSION_PRICES[String(type || "").toLowerCase()] || null;
}

function submissionAmountRaw(type) {
  const price = submissionPrice(type);
  return price ? (BigInt(price) * 10n ** BigInt(ASIQ_DECIMALS)).toString() : null;
}
...
async function getJettonWalletAddress(ownerAddress) {
  const client = new TonClient({ endpoint: "https://toncenter.com/api/v2/jsonRPC", apiKey: TONCENTER_API_KEY || undefined });
...
function createJettonTransferPayload({ amountRaw, recipient, responseAddress, comment }) {
  ...
  return beginCell().storeUint(JETTON_TRANSFER_OP, 32).storeUint(0, 64).storeCoins(BigInt(amountRaw))
    .storeAddress(Address.parse(recipient)).storeAddress(Address.parse(responseAddress)).storeBit(0)
    .storeCoins(SUBMISSION_FORWARD_TON).storeBit(1).storeRef(forwardPayload).endCell().toBoc().toString("base64");
}

function normalizeJettonAction(action) { return action?.JettonTransfer || action?.jetton_transfer || null; }

async function findSubmissionPayment({ sourceWallet, comment, amountRaw, createdAt }) {
  ...
  url.searchParams.set("limit", "100"); url.searchParams.set("subject_only", "false");
  ...
  for (const event of Array.isArray(data?.events) ? data.events : []) {
    ...
    for (const action of Array.isArray(event?.actions) ? event.actions : []) {
      const transfer = normalizeJettonAction(action); if (!transfer) continue;
      ...
      if (successful && sender === source && recipient === merchant && jettonMaster === normalizeAddress(ASIQ_MASTER)
...
async function sendSubmissionNotification({ telegramUser, submission, payment }) {
  if (!ADMIN_CHAT_ID) return;
  const username = telegramUser.username ? `@${telegramUser.username}` : "нет username";
  const typeLabel = submission.type === "track" ? "Track" : "Instrumental";
  const text = ["🎵 Новая заявка Submit Track", "", `👤 Артист: ${submission.artist_name}`,
    `🎼 Название: ${submission.title}`, `📂 Тип: ${typeLabel}`, `🔗 Telegram: ${username}`,
    `🆔 Telegram ID: ${telegramUser.id}`, `👛 Кошелёк: ${submission.wallet_address}`,
    `💰 Оплачено: ${submission.amount_asiq} ASIQ`, `📦 Заявка: ${submission.id}`,
    "✅ Статус: оплачено, ожидает модерации", "", `🔐 Транзакция: ${payment.hash || "найдена в блокчейне"}`,
    SUBMISSION_FILES_CONTACT_URL ? `📩 Контакт для файлов: ${SUBMISSION_FILES_CONTACT}` : ""].filter(Boolean).join("\n");
  try { await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({chat_id:ADMIN_CHAT_ID,text}) }); }
  catch (error) { console.error("Submission notification error:", error); }
}
...
async function initMusicSubmissionsTable() {
  await pool.query(`CREATE TABLE IF NOT EXISTS music_submissions (
    id UUID PRIMARY KEY, telegram_id BIGINT NOT NULL, wallet_address TEXT NOT NULL,
    artist_name TEXT NOT NULL, title TEXT NOT NULL,
    type TEXT NOT NULL CHECK (type IN ('track','instrumental')),
    amount_asiq NUMERIC(30,9) NOT NULL, amount_raw NUMERIC(40,0) NOT NULL,
    comment TEXT NOT NULL UNIQUE,
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','paid','expired','approved','rejected')),
    tx_hash TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), expires_at TIMESTAMPTZ NOT NULL,
    paid_at TIMESTAMPTZ, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`);
  await pool.query(`CREATE INDEX IF NOT EXISTS music_submissions_user_idx ON music_submissions (telegram_id,created_at DESC)`);
}
...
app.post("/api/submissions/create", requireTelegramUser, async (req, res) => {
  const walletAddress = normalizeAddress(req.body?.walletAddress);
  const artistName = cleanSubmissionText(req.body?.artistName, 60);
  const title = cleanSubmissionText(req.body?.title, 120);
  const type = String(req.body?.type || "").toLowerCase();
  const amountAsiq = submissionPrice(type);
  const amountRaw = submissionAmountRaw(type);
  if (!walletAddress || artistName.length < 2 || title.length < 2 || !amountAsiq || !amountRaw) {
    return res.status(400).json({ error: "Wallet, artist name, title and valid type are required" });
  }
  const id = crypto.randomUUID();
  const comment = `ASIQPAI-SUBMIT-${id.slice(0,8).toUpperCase()}`;
  try {
    const inserted = await pool.query(`INSERT INTO music_submissions
      (id,telegram_id,wallet_address,artist_name,title,type,amount_asiq,amount_raw,comment,expires_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW()+INTERVAL '30 minutes') RETURNING *`,
      [id,req.telegramUser.id,walletAddress,artistName,title,type,amountAsiq,amountRaw,comment]);
    const jettonWalletAddress = await getJettonWalletAddress(walletAddress);
    const payload = createJettonTransferPayload({ amountRaw, recipient:MERCHANT_WALLET, responseAddress:walletAddress, comment });
    return res.json({ ok:true, orderId:id, amountAsiq, amountRaw, comment, expiresAt:inserted.rows[0].expires_at,
      transaction:{ validUntil:Math.floor(Date.now()/1000)+600, messages:[{address:jettonWalletAddress,amount:"80000000",payload}] } });
  } catch (error) {
...
app.post("/api/submissions/verify", requireTelegramUser, async (req,res)=>{
  const orderId=String(req.body?.orderId||"").trim();
  if(!orderId) return res.status(400).json({error:"Order ID is required"});
  const client=await pool.connect();
  try{
    await client.query("BEGIN");
    const result=await client.query(`SELECT * FROM music_submissions WHERE id=$1 AND telegram_id=$2 FOR UPDATE`,[orderId,req.telegramUser.id]);
    const submission=result.rows[0];
    if(!submission){await client.query("ROLLBACK");return res.status(404).json({error:"Submission not found"});}
    if(submission.status==="paid"){await client.query("COMMIT");return res.json({ok:true,status:"paid",submissionId:submission.id});}
    if(submission.status==="expired"||new Date(submission.expires_at).getTime()<Date.now()){
      await client.query(`UPDATE music_submissions SET status='expired' WHERE id=$1`,[submission.id]);
      await client.query("COMMIT");return res.status(410).json({error:"Submission expired"});
    }
    const payment=await findSubmissionPayment({sourceWallet:submission.wallet_address,comment:submission.comment,amountRaw:String(submission.amount_raw),createdAt:submission.created_at});
    if(!payment){await client.query("COMMIT");return res.status(202).json({ok:true,status:"pending"});}
    await client.query(`UPDATE music_submissions SET status='paid',tx_hash=$2,paid_at=NOW(),updated_at=NOW() WHERE id=$1`,[submission.id,payment.hash]);
    await client.query("COMMIT");
    await sendSubmissionNotification({telegramUser:req.telegramUser,submission,payment});
    return res.json({ok:true,status:"paid",submissionId:submission.id,filesContact:SUBMISSION_FILES_CONTACT,filesContactUrl:SUBMISSION_FILES_CONTACT_URL});
  }catch(error){try{await client.query("ROLLBACK")}catch{};console.error("Submission verify error:",error);return res.status(500).json({error:"Could not verify submission payment"});}
...
await initMusicSubmissionsTable();
